package com.mycabuser.Utils.ProgressBarCustom;

public class Api {

    public static final String BASE_URL = "https://developerdeepika.com/mycab/appservice/process.php?";

    public static final String genrate_otp="action=genrate_otp";
    public static final String verify_otp="action=sign_in";
    public static final String signup="action=signup";
    public static final String login="action=login";
    public static final String show_driver_responce="action=show_driver_responce";
    public static final String show_driver_to_user="action=show_driver_to_user";
}
